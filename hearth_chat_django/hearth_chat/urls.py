...
# 외부 스토리지(S3 등)가 media 파일을 처리하는 것이 이상적
if settings.DEBUG:
    urlpatterns += static(settings.MEDIA_URL, document_root=settings.MEDIA_ROOT)

# --- 3. React 앱 서빙 (Fallback) ---
# 위에서 정의한 API나 admin 경로가 아닐 경우, 모든 요청을 React의 index.html로 전송
# 이렇게 하면 React Router가 브라우저에서 경로를 처리
# Django admin 경로는 React 앱으로 전송하지 않음
