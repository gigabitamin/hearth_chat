...
const ChatBox = () => {
  ...
  const initializeTTSService = () => {
    try {
      ...
      // 음성 설정을 지연시켜 브라우저가 음성 목록을 완전히 로드한 후 설정
      const setupVoice = () => {
        const voices = ttsService.getVoices();
        if (voices.length > 0) {
          // Google 한국의 (ko-KR) 음성 우선 선택 - 더 정확한 매칭
          const googleKoreanVoice = voices.find(voice =>
          ...
          let selectedGoogleVoice = googleKoreanVoice;
          if (!selectedGoogleVoice) {
            selectedGoogleVoice = voices.find(voice =>
              voice.name.includes('Google') && voice.lang.includes('ko')
            );
          }

          // Microsoft Heami 한국어 음성
          const microsoftKoreanVoice = voices.find(voice =>
          ...
          // 음성 설정 (우선순위: Google 한국의 > Microsoft 한국어 > 일반 한국어 > 첫 번째 음성)
          const selectedVoice = selectedGoogleVoice || microsoftKoreanVoice || koreanVoice || voices[0];
          setTtsVoice(selectedVoice);

          console.log('음성 설정 완료:', {
            selectedVoice: selectedVoice?.name,
            selectedLang: selectedVoice?.lang,
            googleKoreanVoice: selectedGoogleVoice?.name,
            microsoftKoreanVoice: microsoftKoreanVoice?.name,
            koreanVoice: koreanVoice?.name,
            totalVoices: voices.length
          });
        } else {
          console.log('음성 목록이 아직 로드되지 않았습니다. 다시 시도합니다.');
          // 1초 후 다시 시도
  ...
  const speakAIMessage = async (message) => {
    try {
      ...
      console.log('ttsVoice:', ttsVoice?.name);
      console.log('ttsVoice lang:', ttsVoice?.lang);
      console.log('ttsRate:', ttsRate);
      console.log('ttsPitch:', ttsPitch);
      ...
      console.log('현재 선택된 음성:', ttsVoice?.name || '없음');
      ...
      // 음성이 설정되지 않았으면 TTS 서비스의 기본 음성 사용
      let voiceToUse = ttsVoice;
      if (!voiceToUse) {
        voiceToUse = ttsService.voiceSettings.voice;
        console.log('음성이 설정되지 않아 TTS 서비스 기본 음성 사용:', voiceToUse?.name);
      }

      // 음성이 여전히 없으면 첫 번째 사용 가능한 음성 사용
      if (!voiceToUse && voices.length > 0) {
        voiceToUse = voices[0];
        console.log('기본 음성도 없어서 첫 번째 사용 가능한 음성 사용:', voiceToUse?.name);
      }

      if (!voiceToUse) {
        console.error('사용 가능한 음성이 없습니다.');
        return;
      }
