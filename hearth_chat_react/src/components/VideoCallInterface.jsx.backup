...
const VideoCallInterface = ({ roomId, userId, onCallEnd, webSocket }) => {
    ...
    // WebSocket 참조 가져오기
    const getWebSocket = () => {
        console.log('[화상채팅] getWebSocket 호출됨:', {
            hasWebSocketProp: !!webSocket,
            webSocketReadyState: webSocket?.readyState,
            hasWindowWebSocket: !!window.chatWebSocket,
            windowWebSocketReadyState: window.chatWebSocket?.readyState
        });

        // props로 받은 WebSocket을 우선적으로 사용
        if (webSocket && webSocket.readyState === WebSocket.OPEN) {
            console.log('[화상채팅] props WebSocket 사용:', webSocket.readyState);
            return webSocket;
        }
        // fallback: window.chatWebSocket 사용
        if (window.chatWebSocket && window.chatWebSocket.readyState === WebSocket.OPEN) {
            console.log('[화상채팅] window.chatWebSocket 사용:', window.chatWebSocket.readyState);
